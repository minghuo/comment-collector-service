 ...
 * 小红书 - 中科天玑供应商。
 *
 * <p>Bean 名称 = {@code xhs_golaxy}。
 *
 * <p>实现对照 {@code auto-task-web} 的 {@code XhsCrawler}：
 * 内部失败重试 3 次（每次间隔 1s），游标型翻页，字段解析走 {@link Comment#buildFromGolaxy(String)}。
 *
 * <h3>入参（{@code request.extra}）</h3>
 * <ul>
 *   <li>{@code mid} —— 笔记 id；为空时回退 {@code request.targetId}（必填）</li>
...
public class XhsGolaxyCrawlerProvider implements CommentProvider {
    ...
    public CommonEntity<Comment> fetchComments(CommentCollectRequest request) {
        ...
        log.info("[xhs-golaxy] 采集开始: mid={} commentId={} cursor={}", mid, commentId, cursor);
        ...
        if (StringUtils.isNotBlank(commentId)) {
            apiUrl = String.format("%s/reply/xiaohongshu/v2?apiKey=%s&video_id=%s&comment_id=%s",
                    GolaxyConstants.COMMENT_BASE_URL, GolaxyConstants.COMMENT_API_KEY, mid, commentId);
        } else {
            apiUrl = String.format("%s/xiaohongshu/v2?apiKey=%s&video_id=%s",
    ...
    private CommonEntity<Comment> fetch(String apiUrl) {
        for (int i = 0; i < RETRY; i++) {
            try {
                ...
                log.warn("[xhs-golaxy] 第 {}/{} 次请求失败: {}", i + 1, RETRY, e.getMessage());
            }
            CommonTools.sleep(1000);
        }
